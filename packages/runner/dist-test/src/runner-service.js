...
class RunnerService {
    ...
    async runForever() {
        console.log(`[runner] polling loop started — interval=${this.config.pollIntervalMs}ms ` +
    ...
    async processTask(task) {
        ...
        try {
            ...
            if (watchResult.final.phase !== 'succeeded' || !parsedResult.success) {
                const failureSummary = buildFailureSummary(task, driver.name, branch, jobName, watchResult.final.phase, parsedResult.error, combinedLogs);
            ...
            const pullRequest = await this.deps.git.openPR(branch, `[${task.id}] ${task.title}`, buildPullRequestBody(task, driver.name, branch, jobName, commitSha, parsedResult.summary));
            await this.deps.coord.updateTask(task.id, {
                status: 'review',
                metadata: {
                    branch,
            ...
            console.log(`${taskContext} moved to review via ${pullRequest.url} using ${driver.name}.`);
        }
        catch (error) {
    ...
    async blockTask(task, errorBody, metadata) {
        try {
            await this.deps.coord.updateTask(task.id, {
                status: 'blocked',
                metadata,
            ...
            console.error(`[runner] Failed to mark task ${task.id} blocked: ${formatError(updateError)}`, updateError);
        }
        try {
            ...
            console.error(`[runner] Failed to send orch blocker for task ${task.id}: ${formatError(messageError)}`, messageError);
        }
    }
}
exports.RunnerService = RunnerService;
function buildPullRequestBody(task, driverName, branch, jobName, commitSha, summary) {
    return [
        `Task: ${task.id}`,
        `Owner: ${task.owner}`,
        `Driver: ${driverName}`,
...
function buildFailureSummary(task, driverName, branch, jobName, phase, parsedError, logs) {
