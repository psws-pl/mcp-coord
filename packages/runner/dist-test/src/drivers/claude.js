...
exports.claudeDriver = exports.ClaudeDriver = void 0;
...
class ClaudeDriver {
    name = 'claude';
    buildJobSpec(task, branch, options) {
...
exports.claudeDriver = new ClaudeDriver();
function buildProjectVolume() {
    return {
...
function buildInitContainer(task, branch, imagePullPolicy) {
    return {
...
function buildMainContainer(task, branch, imagePullPolicy, secretName) {
    return {
        name: 'claude',
        image: CLAUDE_IMAGE,
        imagePullPolicy,
...
function buildProjectVolumeMount() {
    return {
        name: PROJECT_VOLUME_NAME,
        mountPath: PROJECT_VOLUME_MOUNT_PATH,
    };
}
function buildGitIdentityEnv(task, branch) {
    return [
        {
            name: 'TASK_ID',
            value: task.id,
...
function buildTaskPrompt(task, branch) {
    return [
        `Task ID: ${task.id}`,
        `Title: ${task.title}`,
        `Owner: ${task.owner}`,
...
function matchSingleLine(value, pattern) {
...
function extractChangedFiles(output) {
    const sectionMatch = output.match(/^Changed files:\s*([\s\S]*?)(?:\n[A-Z][A-Za-z ]+:\s|\n\n|$)/m);
...
function looksLikeRuntimeFailure(output) {
    return [
        /npm ERR!/i,
...
function extractFailureReason(output) {
    const matchedLine = output
        .split('\n')
        .map((line) => line.trim())
        .find((line) => [/npm ERR!/i, /^fatal:/i, /^error:/i, /invalid api key/i, /anthropic_api_key/i].some((pattern) => pattern.test(line)));
    return matchedLine || 'Claude job failed without a structured error message.';
}
function buildSummary(summary, changedFiles) {
    const normalizedSummary = summary?.trim();
    if (changedFiles.length === 0) {
        return normalizedSummary;
    }
    const changedFilesSummary = changedFiles.length <= 5
        ? changedFiles.join(', ')
